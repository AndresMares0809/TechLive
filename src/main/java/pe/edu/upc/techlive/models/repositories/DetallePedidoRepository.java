...
public interface DetallePedidoRepository extends JpaRepository<DetallePedido, Integer> {
	...
	@Query("select dp from DetallePedido dp where dp.cliente.id = ?1 and dp.isConfirmed = ?2 order by dp.id asc")
	List<DetallePedido> findByClienteAndConfirmed(Integer id, Boolean isConfirmed) throws Exception;
	
	@Query("select dp from DetallePedido dp where dp.producto.vendedor.id = ?1 and dp.isConfirmed = ?2 order by dp.pedido.id asc")
	List<DetallePedido> findByVendedorAndConfirmed(Integer id, Boolean isConfirmed) throws Exception;
